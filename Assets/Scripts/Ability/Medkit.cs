    ...
    public override void UpdateAbility(bool button)
    {
        ...
        if (_buttonDown && _timer + _abilityInterval <= BoltNetwork.ServerFrame && (state.Energy - _cost) >= 0)
        {
            ...
            if(entity.IsOwner)
            {
                ...
                {
                    BoltNetwork.Instantiate(BoltPrefabs.Medkit, hit.point, Quaternion.identity).GetComponent<AOE>().launcher = GetComponent<PlayerMotor>();
                }
                else
                {
                    BoltNetwork.Instantiate(BoltPrefabs.Medkit, transform.position, Quaternion.identity).GetComponent<AOE>().launcher = GetComponent<PlayerMotor>();
                }
            }
        }
    }
