...
public class Grenade : EntityBehaviour<IPhysicState>
{
    ...
    private float _bounceThreshold = 0.2f;
...
private void OnCollisionEnter(Collision collision)
    {
        ...
        x = (Mathf.Abs(x) < _bounceThreshold) ? 1f : -Mathf.Abs(x);
        y = (Mathf.Abs(y) < _bounceThreshold) ? 1f : -Mathf.Abs(y);
        z = (Mathf.Abs(z) < _bounceThreshold) ? 1f : -Mathf.Abs(z);
